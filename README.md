...
- Python's numbers are set by `os.urandom`: each call is a system call costing
  about 1 µs on macOS whether it returns 7 bytes or 160. That's most of the
  cost of every random format, and why Snowflake, which needs no randomness,
  is Python's fastest. The standard library has no user-space CSPRNG, and
  idgenkit doesn't buffer random bytes (see Security).
- Relative IDs cost about as much as ULIDs because every implementation keeps
  the tags of recently used keys in a small cache, so the HMAC only runs the
  first time a key is seen. An uncached tag is two SHA-256 blocks: about 80 ns
  in C and Rust, which use the CPU's SHA-256 instructions (ARMv8 or x86
  SHA-NI) when present, and about 450 ns with the portable fallback.
...
- Relative IDs reveal their creation time and which IDs belong to the same key.
  ...
  against the RFC 4231 HMAC vectors on both the hardware and portable paths.
  The per-generator tag cache holds keys and tags, never the secret.
