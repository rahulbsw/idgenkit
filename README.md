...
| | ULID new | ULID monotonic | ULID parse | Snowflake | Nano ID (21) |
|---|---|---|---|---|---|
| C core | 48 | 14 | 12 | 244 | 194 |
| Rust | 56 | 27 | 7 | 244 | 208 |
| Go | 98 | 35 | 12 | 244 | 231 |
| Java 24 | 194 | 22 | 27 | 244 | 188 |
| Python 3.12 | 1067 | 321 | 2023 | 312 | 1016 |

**Databases**

| | ULID | ULID monotonic | Snowflake | Nano ID | baseline |
|---|---|---|---|---|---|
| PostgreSQL 17 (ns/row) | 68 | 40 | 203¹ | 90 | `gen_random_uuid()` 522 |
| MySQL 9.2 (ns/call) | 84 | 55 | 273 | 239 | `UUID()` 56 |
| Redis 8.4 (req/s, 50 clients, pipeline 16) | 712k | 717k | 722k | 702k | `PING` 639k |
...
- ¹ PostgreSQL numbers subtract the cost of `count(n)` over `generate_series`.
  ...
  1M rows was 279 ms, which respects the cap.
- Redis throughput is bound by networking and command dispatch: every command
  runs at `PING` speed, so the generator cost doesn't show.
...
- Parallel generation on macOS contends on the system random source's lock
  (Go `ulid.NewParallel` is 322 ns/op versus 98 ns single-threaded). Linux's
