...
Source: [`bench/results/storage-postgres18.txt`](https://github.com/rahulbsw/idgenkit/blob/main/bench/results/storage-postgres18.txt), generated by `bench/doc_tables.py`.
...
Bytes per ID after compression; lower is better. Highlighted cells still need
15 bytes or more per ID.

<!-- bench-table compression -->
...
Source: [`bench/results/storage-compression.txt`](https://github.com/rahulbsw/idgenkit/blob/main/bench/results/storage-compression.txt), generated by `bench/doc_tables.py`.

<!-- /bench-table -->

<figure>
  <img src="assets/compression-1k.svg" alt="Bar chart of compressed bytes per ID at 1,000 IDs per second">
  <figcaption>Compressed size at 1,000 IDs per second. Orange bars are formats that still need 15 bytes or more per ID.</figcaption>
</figure>
...
a column in less. Bold marks layouts within 0.2 bytes of the floor. The last
column shows how many of 100 row groups a lookup for one existing ID has to read
after min/max pruning.
