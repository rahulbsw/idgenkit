...
redis-server --port "$PORT" --save "" --appendonly no --daemonize no \
  --loadmodule "$PWD/build/idgenkit.so" >build/redis-bench.log 2>&1 &
PID=$!
...
for cmd in "PING" "ULID.GENERATE" "ULID.MONOTONIC" "UUIDV4.GENERATE" "UUIDV7.GENERATE" "UUIDV7.MONOTONIC" "SNOWFLAKE.GENERATE" "NANOID.GENERATE" "NANOID.GENERATE 21 0123456789abcdef"; do
